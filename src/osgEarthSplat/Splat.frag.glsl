...
{
    float coverageValue = UNPACK_COVERAGE*texture(oe_splat_coverageTex, oe_splat_covtc).r;
...
vec4 oe_splat_bilinear(in vec2 splat_tc, inout oe_SplatEnv env)
{
    ...
    float value_sw = UNPACK_COVERAGE*value.w;
    float value_se = UNPACK_COVERAGE*value.z;
    float value_ne = UNPACK_COVERAGE*value.y;
    float value_nw = UNPACK_COVERAGE*value.x;
