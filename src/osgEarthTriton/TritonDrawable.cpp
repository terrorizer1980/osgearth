...
#define LC "[TritonDrawable] "
 ...
 //#define OE_TRITION_SMART_HM_UPDATE
...
namespace
{
	...
	{
		PassHeightMapToTritonCallback(TritonContext* triton, CameraData* cam_data) : _TRITON(triton), _cameraData(cam_data), _enable(false), _id(0), _frameNumber(0) { _ptrEnable = const_cast<bool*> (&_enable); };
		virtual void operator()(osg::RenderInfo& renderInfo) const
		{
			//if (_enable)
			{
				_frameNumber++;
				//wait one frame before updating heightmap
				if (_frameNumber == 2)
				{
					_frameNumber = 0;
					//Height map should be updated, disable camera.
					//updateHeightMap below will enable the camera again if updated is requested 
#ifdef OE_TRITION_SMART_HM_UPDATE
					_cameraData->_heightCamera->setNodeMask(0);
					...
					if (_TRITON->ready())
					{
						//std::cout << "HM updated\n";
#ifdef USE_TRITON_CAMERA
						_TRITON->getEnvironment()->SetHeightMap((::Triton::TextureHandle)_id, _heightMapMatrix, 0, _cameraData->_tritonCamera);
#else
	...
	_mapNode(mapNode)
{
	...
	_lodScale = 1.0f;
	if (::getenv("HM_CAM_LOD_SCALE"))
		_lodScale = atof(::getenv("HM_CAM_LOD_SCALE"));
}
...
void
TritonDrawable::dirtyAllCameras()
{
	//_contextDirty.setAllElementsTo(1);
	CameraDataMap::iterator iter = _CameraData.begin();
	while (iter != _CameraData.end())
	{
		iter->second->_dirty = true;
		++iter;
	}
}

void
TritonDrawable::updateHeightMap(osg::RenderInfo& renderInfo, CameraData* cam_data) const
{
	if (!_TRITON->ready())
		return;
	...
	cam_data->_heightCamera->setProjectionMatrix(osg::Matrix::ortho(-horizonDistance, horizonDistance, -horizonDistance, horizonDistance, near, far));
	cam_data->_heightCamera->setViewMatrixAsLookAt(heightCamEye, osg::Vec3d(0.0, 0.0, 0.0), osg::Vec3d(0.0, 0.0, 1.0));

	cam_data->_heightCamera->setLODScale(_lodScale);
	...
	osg::Matrix hMM = cam_data->_heightCamera->getViewMatrix() * cam_data->_heightCamera->getProjectionMatrix() * bias;
	...
	osg::Texture::TextureObject* texObj = cam_data->_heightMap->getTextureObject(renderInfo.getContextID());

	if (texObj)
	{

#ifndef OE_TRITION_SMART_HM_UPDATE
#ifdef USE_TRITON_CAMERA
		_TRITON->getEnvironment()->SetHeightMap((::Triton::TextureHandle)texObj->id(), heightMapMatrix, 0, cam_data->_tritonCamera);
#else
		_TRITON->getEnvironment()->SetHeightMap((::Triton::TextureHandle)_id, cam_data->_heightMapMatrix);
#endif
#else
		PassHeightMapToTritonCallback* cb = dynamic_cast<PassHeightMapToTritonCallback*>(cam_data->_heightCamera->getFinalDrawCallback());
		if (cb)
		{
			if (cb->_frameNumber == 0)
			{
				//cb->_enable = true;
				cb->_id = texObj->id();
				cb->_heightMapMatrix = heightMapMatrix;
				//enable camera
				cam_data->_heightCamera->setNodeMask(0xffffffff);
	...
	mapNode->getParent(0)->insertChild(0, makeFrustumFromCamera(cam_data->_heightCamera));
#endif /* DEBUG_HEIGHTMAP */
}

void TritonDrawable::initialize(osg::RenderInfo& renderInfo)
{
	if (_TRITON->passHeightMapToTriton() && _TRITON->updateHeightMapOnTerrainLoad())
	{
...
CameraData* TritonDrawable::getOrCreateCameraData(osg::RenderInfo& renderInfo)
{
	osg::Camera* cam = renderInfo.getCurrentCamera();
	CameraData * cam_data = NULL;
	std::map<osg::Camera*, CameraData*>::const_iterator iter = _CameraData.find(cam);
	if (iter == _CameraData.end())
	{
		cam_data = new CameraData();
#ifdef USE_TRITON_CAMERA
		cam_data->_tritonCamera = _TRITON->getEnvironment()->CreateCamera();
#endif
		if (_TRITON->passHeightMapToTriton())
			const_cast<TritonDrawable*>(this)->setupHeightMap(renderInfo, cam_data);
		_CameraData[cam] = cam_data;
		//std::cout << "Camera "<< cam->getName() <<  " created at context:" << renderInfo.getContextID() << std::endl;
	}
	else
	{
		cam_data = iter->second;
	} 
	return cam_data;
}

void TritonDrawable::applyAdapters(osg::RenderInfo& renderInfo)
{
	osg::State* state = renderInfo.getState();
	osgEarth::NativeProgramAdapterCollection&adapters = _adapters[renderInfo.getContextID()];
	if (adapters.empty())
	{
...
TritonDrawable::drawImplementation(osg::RenderInfo& renderInfo) const
{
	...
	TritonDrawable* this_instance = const_cast<TritonDrawable*>(this);
	this_instance->initialize(renderInfo);
	
	this_instance->applyAdapters(renderInfo);

	CameraData * cam_data = this_instance->getOrCreateCameraData(renderInfo);
	...
	{
#ifdef USE_TRITON_CAMERA
		if (cam_data->_tritonCamera)
		{
			cam_data->_tritonCamera->SetCameraMatrix(renderInfo.getCurrentCamera()->getViewMatrix().ptr());
			cam_data->_tritonCamera->SetProjectionMatrix(state->getProjectionMatrix().ptr());
		}
#else
...
#ifdef OE_TRITION_SMART_HM_UPDATE
		osg::Vec3d eye, center, up;
		renderInfo.getView()->getCamera()->getViewMatrix().getLookAt(eye, center, up);
		if (_TRITON->getHeightMapUpdateDistance() > 0)
		{
			cam_data->_dirty = cam_data->_dirty || ((eye - cam_data->_lastEye).length() > _TRITON->getHeightMapUpdateDistance());
		}

		if (cam_data->_dirty)
		{
			updateHeightMap(renderInfo, cam_data);
			cam_data->_dirty = false;
			cam_data->_lastEye = eye;
		}
#else
		updateHeightMap(renderInfo, cam_data);
#endif
	...
	if (environment)
	{
		...
		float inv_lod_scale = 1.0f / cam->getLODScale();
		//std::cout << "TRITON_ZOOM:" << inv_lod_scale << "\n";
		environment->SetZoomLevel(inv_lod_scale);
		...
		if (_TRITON->getOcean())
		{
			...
			_TRITON->getOcean()->Draw(renderInfo.getView()->getFrameStamp()->getSimulationTime(), true, true, true, (void*)(&cid), cam_data->_tritonCamera);
			//_TRITON->getOcean()->UpdateSimulation(renderInfo.getView()->getFrameStamp()->getSimulationTime());
			//_TRITON->getOcean()->DrawConcurrent(renderInfo.getView()->getFrameStamp()->getSimulationTime(), true, true, true, (void*)(&cid), cam_data->_Cam);
			//_TRITON->getOcean()->PostDrawConcurrent();
#else
	...
	state->dirtyAllModes();
	//state->resetVertexAttributeAlias(false, 8);
...
void TritonDrawable::setupHeightMap(osg::RenderInfo& info, CameraData* cam_data)
{
	...
	cam_data->_heightMap = new osg::Texture2D;
	cam_data->_heightMap->setTextureSize(textureSize, textureSize);
	cam_data->_heightMap->setInternalFormat(internalFormat);
	cam_data->_heightMap->setSourceFormat(sourceFormat);
	cam_data->_heightMap->setFilter(osg::Texture2D::MIN_FILTER, osg::Texture2D::LINEAR);
	cam_data->_heightMap->setFilter(osg::Texture2D::MAG_FILTER, osg::Texture2D::LINEAR);

	// Create its camera and render to it
	cam_data->_heightCamera = new osg::Camera;
	cam_data->_heightCamera->setReferenceFrame(osg::Transform::ABSOLUTE_RF_INHERIT_VIEWPOINT);
	cam_data->_heightCamera->setClearMask(GL_DEPTH_BUFFER_BIT | GL_COLOR_BUFFER_BIT);
	cam_data->_heightCamera->setClearColor(osg::Vec4(-1000.0, -1000.0, -1000.0, 1.0f));
	cam_data->_heightCamera->setViewport(0, 0, textureSize, textureSize);
	cam_data->_heightCamera->setRenderOrder(osg::Camera::PRE_RENDER);
	cam_data->_heightCamera->setRenderTargetImplementation(osg::Camera::FRAME_BUFFER_OBJECT);
	cam_data->_heightCamera->setImplicitBufferAttachmentMask(0, 0);
	cam_data->_heightCamera->attach(osg::Camera::COLOR_BUFFER, cam_data->_heightMap);

	cam_data->_heightCamera->setCullMask(~TRITON_OCEAN_MASK);
	cam_data->_heightCamera->setAllowEventFocus(false);
	cam_data->_heightCamera->setFinalDrawCallback(new PassHeightMapToTritonCallback(_TRITON.get(), cam_data));
	...
	osg::StateSet* stateSet = cam_data->_heightCamera->getOrCreateStateSet();
	...
	cam_data->_heightCamera->addChild(mapNode->getTerrainEngine());

	osg::Group* root = osgEarth::findTopMostNodeOfType<osg::Group>(mapNode);
	root->addChild(cam_data->_heightCamera.get());

#ifdef DEBUG_HEIGHTMAP
	mapNode->getParent(0)->addChild(CreateTextureQuadOverlay(cam_data->_heightMap, 0.65, 0.05, 0.3, 0.3));
	mapNode->getParent(0)->insertChild(0, makeFrustumFromCamera(cam_data->_heightCamera));
#endif /* DEBUG_HEIGHTMAP */
}
