...
#else
			_TRITON->getEnvironment()->SetHeightMap((::Triton::TextureHandle)texObj->id(), heightMapMatrix);
#endif*/
...
TritonDrawable::drawImplementation(osg::RenderInfo& renderInfo) const
{
    ...
    if ( _TRITON->passHeightMapToTriton() && _TRITON->updateHeightMapOnTerrainLoad())
    {
		if (!_terrainChangedCallback.valid())
		{
			...
			if (_mapNode.lock(mapNode))
			{
				_terrainChangedCallback = new OceanTerrainChangedCallback(const_cast<TritonDrawable*>(this));
	...
	if (_TRITON->passHeightMapToTriton())
	{
		...
		//bool dirty = /*cam_data->_dirty ||*/
		//	(cam_data->_viewMatrix != renderInfo.getView()->getCamera()->getViewMatrix()) ||
		//	(cam_data->_projMatrix != renderInfo.getView()->getCamera()->getProjectionMatrix());
